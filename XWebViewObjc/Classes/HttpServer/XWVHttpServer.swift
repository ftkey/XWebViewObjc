...
@objc public class XWVHttpServer : NSObject {
    fileprivate var socket: CFSocket!
    private var connections = Set<XWVHttpConnection>()
    private let overlays: [URL]
    @objc private(set) var port: in_port_t = 0

    @objc public var rootURL: URL {
        return overlays.last!
    }
    @objc public var overlayURLs: [URL] {
        return overlays.dropLast().reversed()
    }

    @objc public init(rootURL: URL, overlayURLs: [URL]?) {
    ...
    @objc public convenience init(rootURL: URL) {
    ...
    @objc public func start(port: in_port_t = 0) -> Bool {
        if port == 0 {
    ...
    @objc public  func suspend(_: NSNotification!) {
        close()
        log("+HTTP server is suspended")
    }
    @objc public  func resume(_: NSNotification!) {
        if listen(on: port) {
